#define MARLIN_NAMESPACE_NAME flash_marlin_dp4
/*
...
MarlinFuncPtr get_marlin_kernel(const vllm::ScalarType a_type,
  ...
  if (a_type == vllm::kBFloat16 && b_type == vllm::kU4 &&
      c_type == vllm::kBFloat16 && s_type == vllm::kBFloat16 &&
      thread_m_blocks == 1 && thread_n_blocks == 8 && thread_k_blocks == 8 &&
      m_block_size_8 && has_zp && group_blocks == 2 && threads == 256 &&
      !is_zp_float && stages == 4)
    kernel = Marlin<vllm::kBFloat16.id(), vllm::kU4.id(), vllm::kBFloat16.id(),
                    vllm::kBFloat16.id(), 256, 1, 8, 8, true, 4, 2, false>;
  if (a_type == vllm::kBFloat16 && b_type == vllm::kU4 &&
      c_type == vllm::kBFloat16 && s_type == vllm::kBFloat16 &&
      thread_m_blocks == 1 && thread_n_blocks == 8 && thread_k_blocks == 4 &&
      m_block_size_8 && has_zp && group_blocks == 2 && threads == 128 &&
      !is_zp_float && stages == 4)
    kernel = Marlin<vllm::kBFloat16.id(), vllm::kU4.id(), vllm::kBFloat16.id(),
                    vllm::kBFloat16.id(), 128, 1, 8, 4, true, 4, 2, false>;
  if (a_type == vllm::kBFloat16 && b_type == vllm::kU4 &&
      c_type == vllm::kBFloat16 && s_type == vllm::kBFloat16 &&
      thread_m_blocks == 1 && thread_n_blocks == 4 && thread_k_blocks == 8 &&
      m_block_size_8 && has_zp && group_blocks == 2 && threads == 128 &&
      !is_zp_float && stages == 4)
    kernel = Marlin<vllm::kBFloat16.id(), vllm::kU4.id(), vllm::kBFloat16.id(),
                    vllm::kBFloat16.id(), 128, 1, 4, 8, true, 4, 2, false>;
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* b_bias,
  ...
  STD_TORCH_CHECK(major_capability == 8 && minor_capability == 6,
  ...
  STD_TORCH_CHECK(prob_n == 2560 && prob_k == 320 && top_k == 1 &&
...
torch::stable::Tensor flash_marlin_dp4_gemm(
...
STABLE_TORCH_LIBRARY_IMPL(flash_marlin_dp4, CUDA, m) {
  m.impl("gemm", TORCH_BOX(&flash_marlin_dp4_gemm));
}
